...
tricky concept to grasp. However, the *law of total probability* gives the
...
We, however, want to know the *observations* that each model predicts. To
...
of observations we should expect, viewing each model on its own is of limited
value. Instead, we can compare the two models and see how the predicted
observations for each model differ. 

To do this, we can use the `visual_compare()` function.
...
arbitrary value—for example, 0, and set `n` to the planned sample size for our
experiment.
