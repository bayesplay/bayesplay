...
The only thing left is to calculate the Bayes factor.
...
distribution parametrised in terms of *d* and *n*. We specify a null
...
Performing the calculation as above yields a Bayes factor of ~`r round(bf,2)`,
...
same value reported by @rouderbf.

Although the Bayes factor outlined above is parametrised in terms of the effect
size *d*, it's also possible to perform the calculation directly on the *t*
...
value reported by @rouderbf, and the same value reported above. 
