...
parameters (mean and standard deviation). It might then be useful to ask whether
the Bayes factor conclusion depends on those exact choices.
...
rr <- bfrr(
  ...
  parameters = list(sd = c(1, 50)), # vary sd from 1 to 50
  multicore = FALSE
)
```
...
rr <- bfrr(
  ...
  alternative_prior = alternative_prior, # reuse the definition above
  null_prior = null_prior, # reuse the definition above
  parameters = list(scale = c(0.1, 2), location = c(-1, 1)),
  steps = 32,
  multicore = FALSE
)
rr
