...
```{r}
set.seed(2125519)
group1 <- rnorm(n = 15, mean = 25, sd = 15)
group2 <- rnorm(n = 16, mean = 35, sd = 16)
```
...
As with the one-sample case, a *Cauchy* prior is used for the alternative hypothesis. Again, this will need to be appropriately scaled. In one sample case we scaled it by $\sqrt{n}$. In the two-sample case, however, we'll scale it by $\sqrt{\frac{n_1 * n_2}}{n_1 + n_2}$
