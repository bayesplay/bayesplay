...
@rouderbf also provide an extension of their method to the two sample case, although they do not provide a worked example. Instead, we can generate our own example and directly compare the results from the `bayesplay` package with the results from the [`BayesFactor`](https://cran.r-project.org/web/packages/BayesFactor/index.html) package.
