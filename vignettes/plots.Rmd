...
limits are also automatically set so that the full shape of the likelihood
is visible.
...
First, we load the `ggplot2` package to gain access to its styling functions.

```{r}
library(ggplot2)
```

The functions can then be used to modify the output from the `plot()` function.
...
message will be produced. However, this message can be suppressed using
standard `R` methods.
...
It is also possible to change the linetype with `ggplot2::scale_linetype_manual()`,
